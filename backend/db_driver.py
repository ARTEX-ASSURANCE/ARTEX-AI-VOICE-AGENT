...
class ExtranetDatabaseDriver:
    ...
    def enregistrer_fin_appel(self, id_appel: int, resume_appel: Optional[str] = None) -> bool:
        """
        Met à jour l'enregistrement d'un appel avec le timestamp de fin et un éventuel résumé.
        """
        query = """
            UPDATE journal_appels
            SET timestamp_fin = NOW(), resume_appel = %s
            WHERE id_appel = %s AND timestamp_fin IS NULL
        """ # AND timestamp_fin IS NULL pour éviter de mettre à jour plusieurs fois
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(query, (resume_appel, id_appel))
                conn.commit()
            ...
            # log_system_error("db_driver.enregistrer_fin_appel", f"MySQL Error: {err}", err, id_appel_fk=id_appel)
            return False
